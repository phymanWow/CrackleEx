...
typedef struct _pcap_bluetooth_le_lly_header {
    uint8_t le_packet[0];
} __attribute__((packed)) pcap_bluetooth_le_lly_header;
...
void packet_handler_ble_lly(u_char *user, const struct pcap_pkthdr *h, const u_char *bytes) {
    crackle_state_t *state;
    state = (crackle_state_t *)user;
    size_t header_len = sizeof(pcap_bluetooth_le_lly_header);
...
int main(int argc, char **argv) {
    ...
    switch(cap_dlt)
    {
        ...
        case    BLUETOOTH_LE_LL:     
                packet_hander = packet_handler_ble_lly;
                break;
        default:
